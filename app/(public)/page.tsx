...
export default function HomePage() {
  ...
  // Hook 2: fetch de eventos (siempre se monta)
  useEffect(() => {
    fetchEvents();
  }, [fetchEvents]);

  // Hook 3: derivado de events (siempre se ejecuta)
  const sorted = useMemo(() => {
    return [...events]
      ...
      .sort((a, b) => {
        const da = a.dateIso ? new Date(a.dateIso).getTime() : Number.MAX_SAFE_INTEGER;
        const db = b.dateIso ? new Date(b.dateIso).getTime() : Number.MAX_SAFE_INTEGER;
        return da - db;
      });
  }, [events]);
  ...
  if (loading) {
    return (
      <div className="relative min-h-screen w-full bg-black">
        ...
        <div className=" w-full h-full relative flex items-center justify-center animate-pulse text-lg">
          docargan...
        </div>
      </div>
    );
