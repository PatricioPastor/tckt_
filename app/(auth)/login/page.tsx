    ...
    try {
      await authClient.signIn.email(
        ...
        },
        {
          ...
          onError: (ctx) => {
            setErrorMessage(ctx.error.message ?? "No se pudo iniciar sesion.");
            setIsSubmitting(false);
          },
        }
      );
    } catch (error) {
      const fallback = error instanceof Error ? error.message : "No se pudo iniciar sesion.";
    ...
    try {
      await authClient.signIn.social({
        ...
        fetchOptions: {
          ...
          onError: (ctx) => {
            setErrorMessage(ctx.error.message ?? "Fallo el inicio de sesion con Google.");
            setIsSubmitting(false);
          },
        },
      });
    } catch (error) {
      const fallback = error instanceof Error ? error.message : "Fallo el inicio de sesion con Google.";
      setErrorMessage(fallback);
      setIsSubmitting(false);
    }
  };
